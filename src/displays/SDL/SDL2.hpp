...
class SDL2 {
 public:
    ...
    static std::shared_ptr<SDL_Window> SDL2_CreateWindow(const char *title,
        int x, int y, int w, int h, Uint32 flags);
    static std::shared_ptr<SDL_Renderer> SDL2_CreateRenderer(
        SDL_Window *window, int index, Uint32 flags);
    static std::shared_ptr<SDL_Texture> IMG2_LoadTexture(
        SDL_Renderer *renderer, const char *file);
    static void SDL2_QueryTexture(SDL_Texture *texture, Uint32 *format,
        int *access, int *w, int *h);
    static void SDL2_RenderCopy(SDL_Renderer *renderer, SDL_Texture *texture,
        const SDL_Rect *srcrect, const SDL_Rect *dstrect);
    static void SDL2_RenderPresent(SDL_Renderer *renderer);
    static int SDL2_PollEvent(SDL_Event *event);
    static void SDL2_SetRenderDrawColor(SDL_Renderer *renderer, Uint8 r,
    ...
    static void SDL2_RenderFillRect(SDL_Renderer *renderer,
        const SDL_Rect *rect);

    static std::shared_ptr<TTF_Font> TTF2_OpenFont(const char *file,
        int ptsize);
    static std::shared_ptr<SDL_Surface> TTF2_RenderText_Blended(TTF_Font *font,
        const char *text, SDL_Color fg);
    static std::shared_ptr<SDL_Texture> SDL2_CreateTextureFromSurface(
    ...
    static int Mix2_OpenAudio(int frequency, Uint16 format, int channels,
        int chunksize);
};
